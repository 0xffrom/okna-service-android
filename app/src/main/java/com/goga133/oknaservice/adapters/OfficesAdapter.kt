...
class OfficesAdapter(private val array_offices: Array<Office>, private val context: Context) : RecyclerView.Adapter<OfficesAdapter.OfficeHolder>() {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): OfficeHolder {
       val itemView = LayoutInflater.from(parent.context).inflate(R.layout.adapter_office, parent, false)
    ...
    override fun onBindViewHolder(holder: OfficeHolder, position: Int) {
        ...
        holder.addressLayout.setOnClickListener(View.OnClickListener {
            val gmmIntentUri = Uri.parse("geo:${array_offices[position].Coordinates}?q=${array_offices[position].City},${(array_offices[position].Address)}")
        ...
        holder.telephoneLayout.setOnClickListener(View.OnClickListener {
            ...
            val telephoneIntentUri = Uri.parse("tel:${array_offices[position].Mobile.split(',')[0]}")
